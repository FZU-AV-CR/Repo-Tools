...
def build_query(*, query: str | None, filters: list[str], ranges: list[str] | None = None,
                 ...
                 date_field: str = "created") -> str | None:
    ...
    --year/--created-from/--created-to filter on `date_field`, NOT
    necessarily the record's real-world date. `date_field` defaults to
    "created" -- InvenioRDM's own system field for when the record was
    *ingested into the repository*, which is unrelated to (and often much
    later than) when the underlying data was actually produced. For FRAM,
    for example, the real observation date is metadata.observation_time
    (from the FITS DATE-OBS header) -- filtering by --year expecting
    "observed in that year" against the default "created" field silently
    matches nothing if the record was uploaded well after the fact, which
    is the normal case. Pass the field that actually means what you want
    (--date-field on the CLI, or an adapter's own DEFAULT_DATE_FIELD) --
    don't rely on the "created" default meaning "observed"/"produced".
    ...
    if year:
        clauses.append(f"{date_field}:[{year}-01-01T00:00:00 TO {year}-12-31T23:59:59]")
    if created_after or created_before:
        after = created_after or "*"
        before = created_before or "*"
        clauses.append(f"{date_field}:[{after} TO {before}]")
