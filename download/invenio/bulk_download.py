...
async def main_async(args: argparse.Namespace) -> None:
    ...
    date_field = args.date_field or getattr(adapter, "DEFAULT_DATE_FIELD", None) or "created"
    if args.year or args.created_after or args.created_before:
        logger.info(
            "Date filter targets field %r (%s). Pass --date-field to target a different field "
            "if this isn't what you meant -- e.g. the repository's own 'created' field is when "
            "the record was ingested, not necessarily when the underlying data was produced.",
            date_field,
            "from --date-field" if args.date_field else
            ("adapter default" if getattr(adapter, "DEFAULT_DATE_FIELD", None) else "engine default"),
        )
    query = async_download.build_query(
        ...
        created_after=args.created_after, created_before=args.created_before, date_field=date_field,
    )
...
def parse_args() -> argparse.Namespace:
    ...
    sel.add_argument("--date-field", default=None, metavar="FIELD",
                      help="Which indexed field --year/--created-from/--created-to filter on. Defaults "
                           "to the adapter's DEFAULT_DATE_FIELD if it sets one, else the repository's own "
                           "'created' field (record ingestion time, NOT necessarily when the underlying "
                           "data was produced/observed -- e.g. FRAM's real observation date is "
                           "metadata.observation_time). Set this explicitly if the default doesn't mean "
                           "what you want.")
