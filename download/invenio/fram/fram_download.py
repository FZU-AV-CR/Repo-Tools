...
# The real observation date -- confirmed from fram_upload.py's
# build_invenio_metadata(): "observation_time": extracted["observation_time"],
# a full ISO-8601 datetime taken from the FITS DATE-OBS header. This is
# what --year/--created-from/--created-to filter on by default for FRAM,
# NOT the repository's own record-ingestion "created" field (which is
# unrelated to when the exposure was actually taken -- filtering on
# "created" for an observation year will typically match nothing, since
# records are usually uploaded well after the fact).
#
# TODO confirm this is the exact indexed path in a live search response
# (some oarepo/InvenioRDM setups route custom fields under
# "custom_fields.<name>" rather than flat under "metadata.<name>";
# fram_upload.py's own payload nests it directly under "metadata", but
# the search index mapping isn't guaranteed to mirror that 1:1). If a
# --year/--created-from/--created-to run against this default still
# returns nothing where you expect matches, try:
#     --date-field metadata.dates.date   (the "Created"-type DataCite
#                                          date entry, date-only, no time)
...
#   python3 fram_download.py --environment test1 --dry-run --ids 3g7r6-bx383
#   python3 fram_download.py --environment test1 --filter metadata.site=LaPalma --year 2025
#   python3 fram_download.py --environment production --max-concurrency 4 --output-dir "/mnt/data/fram"
