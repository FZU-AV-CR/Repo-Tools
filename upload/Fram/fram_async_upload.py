...
def _compute_footprint(
    ...
    """
    Return a JSON-encoded GeoJSON Polygon string of the image corners for
    OpenSearch geo_shape indexing, with RA remapped to −180..+180°.
    ...
    NOTE: The `footprint` field in metadata.yaml is a plain `keyword` (string),
    not a structured `object` — oarepo's `object` datatype requires a fixed
    `properties` schema and cannot express GeoJSON's variable-depth nested
    coordinate arrays. So the GeoJSON polygon dict is JSON-serialized here
    before being stored. Cesnet can still map this field as `geo_shape` at
    the OpenSearch level, which supports GeoJSON supplied as a string.
