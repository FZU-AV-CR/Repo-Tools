...
model-specific entry point (e.g. itk_upload.py for ITk), which fills in
that model's default paths and calls bulk_async.main(). Running this file
directly (`python3 bulk_async.py`) raises an error pointing to the correct
entry point instead.

Mirrors fram_bulk_async.py as closely as possible: resume via a stats CSV,
...
Differences from fram_bulk_async.py:
  - Work items are discovered by itk_upload.discover_items() (TXT
    metadata files paired with ZIP data files), not a recursive filesystem
    walk for FITS files, so discovery takes --metadata-dir/--data-root
    instead of a single --input-dir.
...
To reuse for a different metadata model: swap the adapter import in
async_upload.py, give the new adapter its own CLI entry point (mirroring
itk_upload.py's _run_via_bulk_async()), and update discover_items(...)'s
keyword arguments below (plus the matching CLI flags) to whatever that
adapter's discover_items() expects.
"""
...
async def main_async(args: argparse.Namespace) -> None:
    ...
    logger.info(
        "Items to upload: %s (environment=%s, max_concurrency=%s, dry_run=%s)",
        len(items), args.environment, args.max_concurrency, args.dry_run,
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Bulk async upload of ATLAS ITk records from TXT metadata files + matching ZIPs."
    )
    parser.add_argument("--metadata-dir", required=True, help="Directory containing the *.txt metadata files (one per record)")
    parser.add_argument("--data-root", required=True, help="Directory containing the matching *.zip data files")
    parser.add_argument(
        "--readme-file", default=None,
        help="Optional shared README.txt uploaded alongside every record's data ZIP. Omit to upload without a README.",
    )
    parser.add_argument(
        "--stats-path",
    ...
    parser.add_argument(
        "--schema-url", default=adapter.DEFAULT_SCHEMA_URL,
...
if __name__ == "__main__":
    raise SystemExit(
        ...
        "engine, driven by a model-specific entry point. For ITk, run:\n"
        "    python3 itk_upload.py [flags]\n"
        "(itk_upload.py imports this module and calls bulk_async.main() "
        "after filling in ITk's default paths.)"
    )


### bulk_async.py is not run directly -- see itk_upload.py for the actual
### entry point and its usage examples (it calls bulk_async.main() after
### filling in ITk's default paths).
#
